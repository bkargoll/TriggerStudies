...
def histograms(fileName, labelOffline, labelL1, labelL2, labelL2p5, labelL3, L1TauPtCut=0, L1JetPtCut=0, L2PtCut=25., L3PtCut=25., MTCut=40., invertMT=False,
    ...
    for bin in etaBinList:
        ...
        tmpL1Tau = ROOT.TGraph(arrayLength)
        tmpL1Tau.SetName("effL1Tau_" + bin[2])
        effL1Tau.append(tmpL1Tau)
                
        tmpL1Jet = ROOT.TGraph(arrayLength)
        tmpL1Jet.SetName("effL1Jet_" + bin[2])
        effL1Jet.append(tmpL1Jet)
        ...
        # over eta
        tmpEtaL1Tau = ROOT.TGraph(arrayLength)
        tmpEtaL1Tau.SetName("effEtaL1Tau_" + bin[2])
        effEtaL1Tau.append(tmpEtaL1Tau)
        
        tmpEtaL1Jet = ROOT.TGraph(arrayLength)
        tmpEtaL1Jet.SetName("effEtaL1Jet_" + bin[2])
        effEtaL1Jet.append(tmpEtaL1Jet)
        ...
        resEtaL1Tau.append(ROOT.TH1F("resEtaL1Tau_" + bin[2], "", 400, -1.0, 1.0))   
        resPhiL1Tau.append(ROOT.TH1F("resPhiL1Tau_" + bin[2], "", 400, -1., 1.))   
        resRL1Tau.append(ROOT.TH1F("resRL1Tau_" + bin[2], "", 400, 0., 1.))      
        resPtL1Tau.append(ROOT.TH1F("resPtL1Tau_" + bin[2], "", 1000, -100., 100.))     
        resRelptL1Tau.append(ROOT.TH1F("resRelptL1Tau_" + bin[2], "", 1000, -5., 5.)) 
        resEtaL1Jet.append(ROOT.TH1F("resEtaL1Jet_" + bin[2], "", 400, -1.0, 1.0))   
        resPhiL1Jet.append(ROOT.TH1F("resPhiL1Jet_" + bin[2], "", 400, -1., 1.))   
        resRL1Jet.append(ROOT.TH1F("resRL1Jet_" + bin[2], "", 400, 0., 1.))      
        resPtL1Jet.append(ROOT.TH1F("resPtL1Jet_" + bin[2], "", 1000, -100., 100.))     
        resRelptL1Jet.append(ROOT.TH1F("resRelptL1Jet_" + bin[2], "", 1000, -5., 5.)) 
    ...
    for event in events:
        ...
        for i in range(len(etaBinList)):
            if doEtaBin[i] == True:
                effL1Tau[i].SetPoint(NTauFilled,tau.pt(),  foundL1Tau)
                effL1Jet[i].SetPoint(NTauFilled,tau.pt(),  foundL1Jet)
                ...
                effEtaL1Tau[i].SetPoint(NTauFilled, tau.pt(),  foundEtaL1Tau)  
                effEtaL1Jet[i].SetPoint(NTauFilled, tau.pt(),  foundEtaL1Jet)  
    ...
    return  effL1Tau, effL1Jet, effL1, effL2, effL2p5, effL3, effL1L2, effL1L2L2p5, effL2L2p5, effL1_HLT, \
            effEtaL1Tau, effEtaL1Jet, effEtaL1, effEtaL2, effEtaL2p5, effEtaL3, effEtaL1L2, effEtaL1L2L2p5, effEtaL2L2p5, effEtaL1_HLT, \
            resEtaL1Tau, resEtaL1Jet, resEtaL1, resPhiL1Tau, resPhiL1Jet, resPhiL1, resRL1Tau, resRL1Jet, resRL1, \
            resPtL1Tau, resPtL1Jet, resPtL1, resRelptL1Tau, resRelptL1Jet, resRelptL1, \
...
def executeCode(cfgFileName):
    ...
    outputFileName = "/user/kargoll/results/TurnOns/forPhdThesis/" + cfg.path + "/turnOnUnbinned_" + cfg.dataset + "_" + offlineLabel + "_L3" + cfg.l3Label[-10:] + "_" + ptThresholds + "_dZ" + str(cfg.dZ) + prongLabel + "_MT" + str(cfg.MT) + mtLabel + offlProngsLabel + invMLabel + "_smB.root"
...
# ## SM Higgs
executeCode("configs/config_DoubleTauJet_2012A.py")
executeCode("configs/config_DoubleTauJet_2012B.py")
executeCode("configs/config_DoubleTauJet_2012Cv3.py")
executeCode("configs/config_DoubleTauJet_2012Cv4.py")
executeCode("configs/config_DoubleTauJet_2012Dpart1.py")
executeCode("configs/config_DoubleTauJet_2012Dpart2.py")
